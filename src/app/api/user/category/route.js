...
export const GET = async (request) => {
    try {
        ...
        // Extract categoryId from the URL params
        ...
        while (attempt < 5) {
            // Get the count of distinct sellers with active and approved products in the specified category
            const sellers = await Product.distinct('sellerID', {
                isApproved: true,
                isActive: true,
                categoryID: new mongoose.Types.ObjectId(categoryId) // Correctly instantiate ObjectId
            });
            ...
            if (sellers.length >= 10) {
                ...
                products = await Product.aggregate([
                    { $match: { isApproved: true, isActive: true, categoryID: new mongoose.Types.ObjectId(categoryId) } },
        ...
        if (populatedProducts.length === 0) {
            return NextResponse.json({
                success: false,
                message: "No products found that are approved, active, and belong to the specified category."
            }, { status: 404 });
        }
