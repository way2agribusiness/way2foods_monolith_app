...
const AdminProductList = () => {
    ...
    const fetchProducts = async () => {
        try {
            ...
            const queryParams = new URLSearchParams({
                ...
                sellerRole: sellerRole,        // Optional: Include sellerRole filter
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/product?${queryParams}`, {
                headers: {
    ...
    useEffect(() => {
        fetchProducts();
    }, [searchTerm, currentPage, sellerRole, selectedCategoryName, selectedSubcategoryName, isActive, isApproved]);
    ...
    const handleEdit = (productId) => {
        ...
        router.push('/admin/addcollections');
    };
                            ...
                            <tr className="border-b">
                                ...
                                <th className="py-2 font-semibold">
                                    ...
                                    >
                                        ...
                                        <option value="apmcConnect">APMC</option>
                                        <option value="fpoConnect">FPO</option>
                                    </select>
                                </th>
                                ...
                                <tr key={product._id} className="border-t hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-2">
                                        <span className={`px-2 py-1 font-semibold rounded 
                                            ${product.sellerID.role === 'admin' ? 'bg-green-200 text-green-800'
                                                : product.sellerID.role === 'apmcConnect' ? ' bg-amber-200 text-amber-800'
                                                    :
                                                    ' bg-blue-200 text-blue-800'
                                            }`}>
                                            {/* {product.sellerID.role.toUpperCase()} */}
                                            {
                                                product.sellerID.role === 'admin' ? product.sellerID.role.toUpperCase()
                                                    :
                                                    product.sellerID.role === 'apmcConnect' ? 'APMC'
                                                        :
                                                        'FPO'
                                            }
                                        </span>
                                    ...
                                    <td className="px-4 py-2">
                                        <div className="flex justify-center gap-4">
                                            ...
                                            <button
                                                className={`${product.isActive ? 'text-green-500 hover:text-green-700' : 'text-red-500 hover:text-red-700'}`}
