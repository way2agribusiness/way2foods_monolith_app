...
const CategoryWiseProduct = ({ categoryName }) => {
    ...
    const category = categoryName === 'Vegetables' ? '678a27fa78c25ec261070be8' :
        categoryName === 'Fruits' ? '678a28d678c25ec261070c5c' :
            '678a295f78c25ec261070ca0'

    const fetchProductsLike = async () => {
        try {
            // Clear the session storage before fetching new data
            sessionStorage.removeItem(`categoryWiseProductData_${categoryName}`);

            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/category?category=${category}`);
            ...
            // Save the fetched data to sessionStorage with a key based on the category
            sessionStorage.setItem(`categoryWiseProductData_${categoryName}`, JSON.stringify(data));
            setProductData(data);
        } catch (error) {
            console.log(error);
        }
    ...
    useEffect(() => {
        // Check if data exists in sessionStorage for the current category
        const cachedData = sessionStorage.getItem(`categoryWiseProductData_${categoryName}`);

        if (cachedData) {
            // If data exists in sessionStorage, use it
            setProductData(JSON.parse(cachedData));
        } else {
            // Ensure fresh data is fetched by clearing the cache when the page is refreshed
            sessionStorage.removeItem(`categoryWiseProductData_${categoryName}`); // Clear cache on refresh
    ...
    return (
        <>
            <div className={`${categoryName === 'Vegetables' ? 'bg-gradient-to-br from-green-400 via-yellow-200 to-green-500'
                :
                categoryName === 'Fruits' ? 'bg-gradient-to-br from-orange-300 via-red-400 to-green-400'
                    :
                    'bg-gradient-to-br from-green-300 via-yellow-400 to-amber-400'
                } p-4`}>
                <h4 className="text-xl font-semibold mb-4">{categoryName}</h4>
