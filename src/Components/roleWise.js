...
const RoleWiseProduct = ({ roleName }) => {
    ...
    const rolenames = roleName === 'admin' ? 'Way2Foods Products' :
        roleName === 'apmcConnect' ? 'APMC Products' : 'FPO Products'

    const fetchProductsRoles = async () => {
        try {
            // Clear the session storage before fetching new data
            sessionStorage.removeItem(`productsLikeData_${roleName}`);

            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/role?rolename=${roleName}`);
            ...
            // Save the fetched data to sessionStorage with a key based on the role
            sessionStorage.setItem(`productsLikeData_${roleName}`, JSON.stringify(data));
            setProductData(data);
        } catch (error) {
            console.log(error);
        }
    ...
    useEffect(() => {
        // Check if data exists in sessionStorage for the current role
        const cachedData = sessionStorage.getItem(`productsLikeData_${roleName}`);
    ...
    return (
        <>
            <div
                className={`${roleName === 'admin' ? 'bg-gradient-to-br from-emerald-300 via-emerald-400 to-lime-400'
                    :
                    roleName === 'apmcConnect' ? 'bg-gradient-to-br from-sky-300 via-sky-200 to-sky-500'
                        :
                        'bg-gradient-to-br from-purple-200 via-pink-300 to-purple-500'
                    } p-4`}
            >
                <h4 className="text-xl font-semibold mb-4">{rolenames}</h4>
...
export default RoleWiseProduct;
