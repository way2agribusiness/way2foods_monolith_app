...
const ProductsLike = () => {
  ...
  useEffect(() => {
    ...
    if (cachedData) {
      ...
      setIsLoading(false); // Set loading to false if using cached data
    } else {
    ...
    const handleRouteChange = () => {
      // Scroll to the top whenever a route change occurs
      window.scrollTo(0, 0);
    };

    // Listen for route change and trigger scroll to top manually
    ...
    return () => {
      // No specific cleanup necessary since we're overriding the `push` method
    };
  }, [router]);
  ...
  const detailsPage = (slug) => {
    // Navigate to the details page with the scroll option
  ...
  const decrement = (productId) => {
    if (incrementNumber === 1) {
      // When the number is 1 and decrement is clicked
      ...
      // If greater than 1, simply decrement
      setIncrementNumber((prev) => {
        const newNumber = prev - 1;
        // Update the cart product length in sessionStorage by productId
        updateCartProductLength(newNumber, productId);
        ...
        {isLoading ? (
          ...
          >
            {productsData.product &&
              productsData.product.map((item, index) => (
                <div key={index} className="bg-white rounded-lg shadow-lg">
                  <div
                    className="w-48 p-1 flex-shrink-0 select-none"
                  ...
                  >
                    <div className="relative">
                      <img
                        ...
                        draggable="false"
                      />
                      <span className={`absolute top-0 left-0 text-white text-xs`}>
                        {item.sellerID.role === 'apmcConnect' ? (
                          <p className="py-1 px-2 rounded-full" style={{ backgroundColor: '#60a5fa' }}>
                            APMC
                          </p>
                        ) : item.sellerID.role === 'fpoConnect' ? (
                          <p className="py-1 px-2 rounded-full" style={{ backgroundColor: '#c084fc' }}>
                            FPO
                          </p>
                        ) : (
                          <p className="py-1 px-2 rounded-full" style={{ backgroundColor: '#34d399' }}>
                            W2F
                          </p>
                        )}
                      </span>
                    ...
                    <div className="flex justify-between items-center text-gray-800">
                      <span className="line-through text-xs text-gray-500">₹{item.cuttedPrice}</span>
                      <span className="font-semibold text-base">₹{item.price}</span>
                    </div>

                    <p
                      className={`text-sm font-medium mt-1 ${
                        parseInt(item.quantity) > 0 ? 'text-green-500' : 'text-red-500'
                      }`}
                    >
